            ...
            switch (message){
                ...
                case "->forward" ->{
                    ...
                    ResultSet rsf = Database.select("chats",
                            "SELECT * FROM "+chatName+
                                    " LEFT JOIN user_info ON user_info.user_id = "+chatName+".sender_username" +
                                    " WHERE message_id = "+forward_id);

                    if(rsf.next()){
                        sender_username = rsf.getInt("sender_username");
                    ...
                    Database.insert("chats",
                            "INSERT INTO "+destination_chat_table_name+
                                    " (sender_username,content,user_id_forwarded_from) " +
                ...
                case "->reply" ->{
                    ...
                    while (true) {
                        ...
                        ResultSet rsf = Database.select("chats",
                                "SELECT * FROM "+chatName+
                                        " JOIN user_info ON user_info.user_id = "+chatName+".sender_username" +
                                        " WHERE message_id = "+reply_message_id);

                        if(rsf.next()){
                            reply_message_id = rsf.getInt("sender_username");
